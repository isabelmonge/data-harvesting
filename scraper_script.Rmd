---
title: "MarketBeat Scraper Script"
author: "Isabel Monge & Marvin Struckmeyer"
date: "`r Sys.Date()`"
output:
  html_document:
    theme: flatly
...
This document contains code to scrape earnings call transcripts from MarketBeat for S&P 500 companies. The scraper uses RSelenium with a Firefox browser running in a Docker container.

## Notes on Usage

Before running this code:

-   Make sure you have the Docker container named "selenium_firefox" running
-   Port 4449 should be mapped from the container to your local machine
-   The process may take several hours to complete for all S&P 500 companies
-   Results are saved incrementally to the "current_transcripts_df" variable and finally to "all_transcripts.csv"

## Load Required Libraries

First, let's load all the packages we need for this project:
...
This function handles the entire scraping workflow for multiple tickers, including restarting the Docker container between each company to prevent server connection issues.
...
## Single Ticker Processing Function

This function handles the web scraping process for a single company ticker by using Selenium to navigate the MarketBeat website.
...
This function extracts and formats the raw transcript text and date from a MarketBeat webpage after navigation is complete. It returns a clean data frame with the company ticker, formatted date, and processed transcript text.
...
extract_transcript <- function(remDr, ticker) {
  ...
  # try xpath first, then look for any date-like text, then just return NA
date_text <- tryCatch({
...
Finally, we get the S&P 500 tickers from the tidyquant package and run the scraper.
...
Since the full scraper can take several hours to run through all S&P 500 companies, here's a simplified example using just three tickers. This demonstrates the typical output structure without requiring extensive processing time.

```{r}
# define list of tickers
sp500 <- 
tickers <- (c("AAPL", "AMZN", "META"))
