...
[Alpha Vantage’s API](https://www.alphavantage.co) offers access to a wide array of financial data, among other things financial metrics for the 500 companies of the S&P 500 that we are interested in. We have developed three approaches to access the data. They all revolve around the central problem of circumventing Alpha Vantage API’s request limits. Alpha Vantage has a hard limit of 25 requests per API key per day. Moreover, Alpha Vantage tracks the user’s IP address and sets the same hard limit. Hence, a simple API key rotation would not work, as this would limit us to only 25 downloads per day. However, we developed a framework to combine a simple API key rotation with regular changes of the IP address, using [Auto Tor](https://github.com/FDX100/Auto_Tor_IP_changer). This is our main approach, which enabled us to get the data for the 500 companies in a fast and efficient way. 

## Auto Tor IP Changer View

Below is how the Auto Tor terminal looks like when running:

![Auto Tor IP Changer Terminal](auto_tor_screenshot.png)


This tool changes one's IP address automatically by restarting the Tor Browser 
at regular specified intervals. For this code, we recommend 75 seconds.
...
We start with this function which verifies that we have a proper Tor connection 
and retrieves the current IP address. It is essential for confirming that we have
a working anonymous connection before making API requests.
...
This function connects to Alpha Vantage's API via Tor and then tries to retrieve
financial data for a specific company. The function includes some error handling
and logging for API rate limits and connection issues. We add a customizable argument
such that the user can choose the number of retry attempts.
...
Here, we transform the JSON responses from Alpha Vantage into nicer, structured 
dataframes. We only use one API endpoint because each endpoint would require us
to do a separate call, costing us one additional request. And the overview endpoint
gives us most of the relevant information about a company's financial health.
The extract_company_metrics function parses financial data into meaningful categories.
...
# some simple scraping: function to get S&P 500 symbols
...
This is our core function that organises the entire data collection process.

It coordinates everything: checking Tor connections, rotating through API keys to
avoid hitting limits, fetching company data, and saving our progress.
When something goes wrong (like a failed request), it adds the company to a retry 
queue and uses a cool exponential backoff - waiting longer between each retry attempt. 
We also added a functionality that saves the progress after each successful company
to batch_progress_temp.csv, so we don't lose data if anything goes wrong.
...
This section loads our manually collected API keys from the environment to access
...
Finally, we execute the data collection process and track progress.
This chunk manages previously collected data, randomly selects new companies to 
process, and updates the combined dataset called sp500_fundamentals_combined.csv.
...
# Remove companies already collected
remaining_symbols <- all_sp500[!all_sp500 %in% already_collected]
cat("Remaining companies to collect:", length(remaining_symbols), "\n")

# HERE: define how many companies to collect in this run
companies_to_collect <- 2  # adjust this number as needed for testing

# randomly sample from remaining symbols
if (length(remaining_symbols) > 0) {
  # ff we have fewer remaining symbols than requested, collect all of them
